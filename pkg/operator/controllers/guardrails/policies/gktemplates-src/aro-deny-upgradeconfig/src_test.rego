...
test_input_allowed_regularuser_upgradeconfig {
  input := { "review": input_upgradeconfig("system:admin","test","UPDATE") }
...
test_delete_allowed_regularuser_upgradeconfig {
  input := { "review": input_upgradeconfig("system:admin","test","DELETE") }
  results := violation with input as input
  count(results) == 0
}
...
test_create_allowed_regularuser_upgradeconfig {
  input := { "review": input_upgradeconfig("system:admin","test","CREATE") }
  results := violation with input as input
  count(results) == 0
}
...
input_upgradeconfig(group, username, operation) = output {
  output = {
    "object": {
        "apiVersion": "upgrade.managed.openshift.io/v1alpha1",
        "kind": "UpgradeConfig",
        "metadata": {
            "creationTimestamp": "2023-05-18T01:01:27Z",
            "generation": 2,
